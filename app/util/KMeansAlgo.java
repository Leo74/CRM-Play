...
public class KMeansAlgo
{
	...
	public static boolean CheckKMeans(int elementArray[], int NumberOfClusters, int newElement, int distance)
	{
		boolean found = false;
...
/* Initializing m */
		...
		for(int i = 0;i<p;++i)
		{
			...
			{
				if (
						((k[i][j] <= newElement && (k[i][j] + distance) >= newElement)) ||
						((k[i][j] >= newElement) && ((k[i][j] - distance) <= newElement))
					)
				{
					System.out.print(k[i][j]+" [" + newElement +" ] " );
					found = true;
				}
				else
				{
		...
		if (found)
		{
			System.out.println("found in cluster with difference of " + distance);
		}
		return found;
	}
...
/* Accepting number of elements */
		System.out.println("Enter the number of elements ");
		int d1[] = new int[]{20, 30, 60, 80, 120, 150, 180, 220};
		int n1 = d1.length;
		int p1 = 3;
		

		CheckKMeans(d1, p1, 50, 10);
		System.exit(0);
